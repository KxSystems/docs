...
<i class="far fa-hand-point-right"></i> _Q for Mortals_ [11.3 Splayed Tables](http://code.kx.com/q4m3/11_IO/#113-splayed-tables), [14 Introduction to Kdb+](http://code.kx.com/q4m3/14_Introduction_to_Kdb+/)
...
'type
```
The reason is that tables that are splayed across a directory must be fully enumerated and not keyed. The solution is to enumerate columns of type varchar before saving the table splayed. This is done with the function [`.Q.en`](/basics/dotq/#qen-enumerate-varchar-cols).
```
q).Q.en[`:dir] tr
date       open  high  low   close volume    sym
-------------------------------------------------
