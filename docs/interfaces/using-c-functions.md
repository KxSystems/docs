...
To make a function `foo` defined in a shared object `bar.so` available in a q session, we use [2:](/basics/filenumbers/#2-c-shared-objects) to load the function dynamically.
```q
q)foo:`bar 2:(`foo;n)
```
