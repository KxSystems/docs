...
Use the [_scan_ adverb](/basics/adverbs/#scan) to build a moving list adding one item at the time from the original table, and the [`_` drop operator](/basics/lists/#_-drop) to discard older values.
...
..
```
`swin2:fwv` finds all the windows using `prev` and [_converge-iterate_](/basics/adverbs/#converge-iterate). To find windows of size 3:
```q
...
1250
```
An alternative to building queries by concatenating strings is to use [parse trees](/basics/parsetrees/). 
```q
...
!!! note 
    ...
    <i class="far fa-hand-point-right"></i> [Glossary](/basics/glossary) for passing by reference and value
