---
title:  Reference architecture for Azure
description: A reference architecture for Microsoft Azure. This classic approach relies on the existing license. To benefit more from the cloud technology, migrate to KX Insights.
author: Sam Tran
date: November 2021
---
...
Often the streaming data is available on a pub-sub component like Kafka,
Solace, etc., all popular for having an open-source interface to kdb+. The
...
choices for <!-- Amazon Web Services --> Microsoft Azure is the size of memory for this instance,
...
The specific design of a gateway can vary in several ways according to
...
It is recommended to use compression on the HDB data residing on Azure
...
Local table updates are very efficient especially if TP sends batch
...
One benefit of storing log messages is the ability to process log
...
The Azure Hypervisor, based on Windows Hyper-V, is the native hypervisor
