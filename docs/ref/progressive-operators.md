...
A progressive operator takes a map as argument and derives a function that evaluates the map, first on the first item/s of its right argument/s, then on the results of **successive** evaluations.

There are two progressive operators, Scan and Over. They have the same syntax and perform the same computation. But where the derivatives of Scan return the result of each evaluation, those of Over return only the last result. 
...
The domain of the progressive operators is functions, lists, and dictionaries that represent finite-state machines. 
...
The derivative of a progressive operator and a unary map is ambivalent. 
...
The number of evaluations the derivative performs is determined (when applied as a binary) by its left argument, or (when applied as a unary) by convergence.
...
The derivative of a progressive operator and a binary map is an ambivalent function. 
...
When the derivative is applied as a binary, the first evaluation applies the map to the derivative’s left argument and the first item of the derivative’s right, i.e. `m[x;first y]`. The result of this becomes the left argument in the next evaluation, for which the right argument is the second item of the derivative’s right argument. And so on. 
...
When the derivative is applied as a unary, and the map is **a function with a known identity element** $I$, then $I$ is taken as the left argument of the first evaluation.
...
The derivative of a progressive operator and a ternary map has the same rank as the map. 
Functions derived by Scan are uniform; functions derived by Over are aggregates. 
...
!!! warning "Progressive derivatives can change type"
...
The derivative of Scan and a non-unary map is a uniform function: for empty right argument/s it returns the generic empty list.
...
The derivative of Over and a non-unary map is an aggregate: it reduces lists and dictionaries to atoms.

For empty right argument/s the atom result depends on the map and, if the derivative is ambivalent, on how it is applied. 

If the map is a **binary function with a known identity element** $I$ and the derivative is applied as a unary, the result is $I$.
...
If the map is a **binary function with no known identity element**, and the derivative is applied as a unary, the result is `()`, the generic empty list.
...
If the map is a **list** and the derivative is applied as a unary, the result is an empty list of the same type as the list. 
