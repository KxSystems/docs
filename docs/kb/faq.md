...
In this table, the [datatype](/basics/datatypes) of the columns is not defined. The first record inserted into the table determines the datatype of each column. Subsequent inserts may only insert values of this type, or else a type error is signalled.

This creates a table in memory.  
<i class="far fa-hand-point-right"></i> [How do I write a table to disk?](#how-do-i-write-a-table-to-disk)

Tables can also be created functionally with q primitive functions.  
<i class="far fa-hand-point-right"></i> [Dictionaries and tables](/basics/dictsandtables), [Q-SQL](/basics/qsql), [Functional SQL](/basics/funsql)
...
```
<i class="far fa-hand-point-right"></i> [Datatypes](/basics/datatypes)
...
<i class="far fa-hand-point-right"></i> [Q-SQL](/basics/qsql)

## How do I replace null values by something else?

Use the [_fill_](/basics/lists/#fill) operator `^`. For instance, the following replaces all nulls in column `amount` by zeroes.
```q
...
...
```
Column names cannot be parameters of a query. Use [functional queries](/basics/funsql) in such cases.
...
20  oil 77
```
<i class="far fa-hand-point-right"></i> [`0:`](/basics/filenumbers/#0-text-files) (Text files)
...
q)save `:trade.txt
```
<i class="far fa-hand-point-right"></i> [`save`](/basics/filewords/#save)
...
<i class="far fa-hand-point-right"></i> [Shebang (Unix)](http://en.wikipedia.org/wiki/Shebang_(Unix))
