...
The first and second methods on interfacing between q and R are covered by the Fusion interfaces [rkdb](rdkb) and [embedR](embedR). The remaining methods are not supported or owned by Kx but are desbut are described [here](R-and-q), the packages and methods outlined here are kdb-Rmath, RServe and RODBC
