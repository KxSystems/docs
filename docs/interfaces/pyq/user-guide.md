...
q              | Python           | Return
---------------|------------------|---------------------------------------
[`neg()`](/basics/arith-integer/#neg) | [`operator.neg()`](https://docs.python.org/3.6/library/operator.html#operator.neg) | the negative of the argument
[`abs()`](/basics/arith-integer/#abs) | [`abs()`](https://docs.python.org/3.6/library/functions.html#abs) | the absolute value
[`signum()`](/basics/arith-integer/#signum) |   | ±1 or 0 depending on the sign of the argument
[`sqrt()`](/basics/arith-float/#sqrt) | [`math.sqrt()`](https://docs.python.org/3.6/library/math.html#math.sqrt) | the square root of the argument
[`exp()`](/basics/arith-float/#exp) | [`math.exp()`](https://docs.python.org/3.6/library/math.html#math.exp) | _e_ raised to the power of the argument
[`log()`](/basics/arith-float/#log)) | [`math.log()`](https://docs.python.org/3.6/library/math.html#math.log)     | the natural logarithm (base e) of the argument
[`cos()`](/basics/trig/#cos) | [`math.cos()`](https://docs.python.org/3.6/library/math.html#math.cos) | the cosine of the argument
[`sin()`](/basics/trig/#sin) | [`math.sin()`](https://docs.python.org/3.6/library/math.html#math.sin) | the sine of the argument
[`tan()`](/basics/trig/#tan) | [`math.tan()`](https://docs.python.org/3.6/library/math.html#math.tan) | the tangent of the argument
[`acos()`](/basics/trig/#acos) | [`math.acos()`](https://docs.python.org/3.6/library/math.html#math.acos) | the arc cosine of the argument
[`asin()`](/basics/trig/#asin) | [`math.asin()`](https://docs.python.org/3.6/library/math.html#math.asin) | the arc sine of the argument
[`atan()`](/basics/trig/#atan) | [`math.atan()`](https://docs.python.org/3.6/library/math.html#math.atan) | the arc tangent of the argument
[`ceiling()`](/basics/arith-integer/#ceiling) | [`math.ceil()`](https://docs.python.org/3.6/library/math.html#math.ceil) | the smallest integer &ge; the argument
[`floor()`](/basics/arith-integer/#floor) | [`math.floor()`](https://docs.python.org/3.6/library/math.html#math.floor) | the largest integer &le; the argument
...
Unlike Python, where caret (`^`) is the binary _xor_ operator, q defines it to denote the [fill](http://code.kx.com/q/basics/lists/#fill) operation that replaces null values in the right argument with the left argument. PyQ follows the q definition:
```python
>>> x = q('1 0N 2') >>> 0 ^ x
k('1 0 2')
```
...
The `each` adverb serves double duty in q. When it is applied to a **function**, it returns a new [derivative](/basics/adverbs) function that expects lists as arguments and maps the original function over those lists. For example, we can write a ‘daily return’ function in q that takes yesterday’s price as the first argument `x`, today’s price as the second `y`, and dividend as the third `z` as follows:
```python
...
k('1 100 30')
```
<i class="far fa-hand-point-right"></i> [_case_](/basics/adverbs/#case) adverb
